 ...
 */
async function getDefaultConfig({ languagesInput, queriesInput, packsInput, extractorsInput, buildModeInput, dbLocation, trapCachingEnabled, debugMode, debugArtifactName, debugDatabaseName, repository, tempDir, codeql, githubVersion, features, logger, }) {
    // This has to be done before languages are loaded
    const extractors = await (0, extractors_1.getExtractors)(codeql, extractorsInput, logger);
 ...
 */
async function loadConfig({ languagesInput, queriesInput, packsInput, extractorsInput, buildModeInput, configFile, dbLocation, trapCachingEnabled, debugMode, debugArtifactName, debugDatabaseName, repository, tempDir, codeql, workspacePath, githubVersion, apiDetails, features, logger, }) {
    let parsedYAML;
