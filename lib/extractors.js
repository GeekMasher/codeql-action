...
exports.TRUSTED_ORGANIZATIONS = ["github", "codeql", "advanced-security"];
/**
...
function parseExtractors(input) {
    ...
    input
        ...
        extractor.path = toolcache.find(extractor.repository, extractor.version || "*");
 ...
 * @returns
 */
async function getExtractors(codeql, input, logger, token) {
    if (logger === undefined) {
        logger = (0, logging_1.getActionsLogger)();
    }
    if (input !== undefined && input !== "") {
        let extractors = parseExtractors(input);
        logger.info(`External Extractors defined, starting download...`);
        extractors.forEach((extractor) => {
            downloadExtractor(extractor, codeql, token || (0, actions_util_1.getRequiredInput)("token"));
        });
        return extractors;
    }
    return [];
 ...
 * @returns
 */
async function downloadExtractor(extractor, codeql, token) {
    ...
    let path = await codeql.resolveExtractor(languages_1.Language.community);
